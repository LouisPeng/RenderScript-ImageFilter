...
public class AutoLevelFilter implements IImageFilter {
    ...
    private static float[] ComputeGamma(int[] lo, int[] md, int[] hi) {
        ...
        for (int i = 0; i < 3; i++) {
            if (lo[i] < md[i] && md[i] < hi[i]) {
                double log = Math.log1p(((float)(md[i] - lo[i])) / ((float)(hi[i] - lo[i])));
                array[i] = (log > 10.0) ? ((float)10.0) : ((log < 0.1) ? ((float)0.1)
                        : ((float)log));
            } else {
                array[i] = 1f;
            }
    ...
    public int[] GetMeanColor(int[][] h) {
        ...
        return new int[] {
                (((int)(array[0] + 0.5f)) & 255), (((int)(array[1] + 0.5f)) & 255),
                (((int)(array[2] + 0.5f)) & 255)
        };
    }
            ...
            for (int j = 0; j < 256; j++) {
                ...
                for (int n = 0; n < 3; n++) {
                    ...
                    if (percent < 0f) {
                        ...
                        double adjust = array[n]
                                + ((rgb[n] - array[n]) * Math.pow(percent
                                        / (hi[n] - percentileColor[n]), gamma[n]));
                        arr[n] = (adjust > 255.0) ? ((int)255.0) : ((adjust < 0.0) ? ((int)0.0)
                                : ((int)adjust));
                    }
                }
                rr[j] = arr[0];
